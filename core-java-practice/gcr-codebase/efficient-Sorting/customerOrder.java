...
public class MergeSortOrders {
    ...
    private static void merge(int[] orderValues, int left, int mid, int right) {
        int n1 = mid - left + 1;
        int n2 = right - mid;
        ...
        for (int i = 0; i < n1; ++i) L[i] = orderValues[left + i];
        for (int j = 0; j < n2; ++j) R[j] = orderValues[mid + 1 + j];
    ...
    public static void main(String[] args) {
        ...
        if (sc.hasNextInt()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                orderValues[i] = Integer.parseInt(token);
            }
